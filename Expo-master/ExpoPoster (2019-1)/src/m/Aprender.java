...
public class Aprender extends JFrame implements ActionListener{
	...
	static JButton menu = new JButton("Regresar Menu/Back to Menu");
	static JButton sig = new JButton("Siguiente/Next");
	static JButton bot3 = new JButton("Borrar/Delete");
	...
	public Aprender(){
		...
		menu.setBounds(31, 400, 230, 23);
		menu.addActionListener(this);
		contentPane.add(menu);
		
		bot3.setBounds(420, 400, 125, 23);
		...
		if(e.getActionCommand().equalsIgnoreCase(menu.getText())) {	
			...
			if(e.getActionCommand().equalsIgnoreCase(sig.getText())) {
				if(getAux() == filas.length) {
					JOptionPane.showMessageDialog(null, "No hay mas vocabulario/No more vocabulary", "Sin elementos/No elements", JOptionPane.WARNING_MESSAGE);
				}else {
					String [] temp;
					//boolean seguir = true;
					if(getAux() == filas.length) {
						JOptionPane.showMessageDialog(null, "No hay mas vocabulario/No more knowledge", "Sin elementos/No elements", JOptionPane.WARNING_MESSAGE);
						...
						if(temp [2].equalsIgnoreCase("F")) {
							...
							setNameImg(temp[0]);
							ImageIcon pal=new ImageIcon("C:\\Users\\BOG-A408-E-016\\Downloads\\Expo-master\\ExpoPoster (2019-1)\\files\\"+getNameImg()+".png");
